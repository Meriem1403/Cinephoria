...
class BookingController extends AbstractController
{
    ...
    public function bookShowtime(Showtime $showtime, Request $request): Response
    {
    ...
    public function formulas(Showtime $showtime, Request $request, EntityManagerInterface $em): Response
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        ...
        if ($specialDefs === []) {
            // Afficher des formules par défaut si la séance n'en a pas en base
            $price = (float) $showtime->getPrice();
            $specialDefs = [
        ...
        if ($request->isMethod('POST')) {
            ...
            for ($i = 0; $i < $standardCount; $i++) {
                ...
                $seat->setIsReserved(true); // ✅ Ajouté
                $em->persist($seat);        // ✅ Ajouté
    ...
    public function confirmation(int $reservationId, EntityManagerInterface $em): Response
    {
        ...
        if (!$reservation) {
            $this->addFlash('error', 'Cette réservation n’existe pas ou a été supprimée.');
            return $this->redirectToRoute('my_reservations');
        }

        $user = $this->getUser();
        if ($reservation->getUser() !== $user && !$this->isGranted('ROLE_ADMIN')) {
            $this->addFlash('error', 'Vous ne pouvez pas accéder à cette réservation.');
            return $this->redirectToRoute('my_reservations');
        }
    ...
    #[Route('/booking/confirmation/{reservationId}/download', name: 'booking_confirmation_download', methods: ['GET'])]
    public function downloadEticket(int $reservationId, EntityManagerInterface $em, Pdf $pdf): Response
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        ...
        $user = $this->getUser();
        if ($reservation->getUser() !== $user && !$this->isGranted('ROLE_ADMIN')) {
            $this->addFlash('error', 'Vous ne pouvez pas télécharger cette réservation.');
            return $this->redirectToRoute('my_reservations');
        }
        ...
        $bannerDataUri = 'data:image/'.pathinfo($heroPath, PATHINFO_EXTENSION).';base64,' . base64_encode(file_get_contents($heroPath));
