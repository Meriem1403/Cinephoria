...
class UserFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $employee->setAvatar('Bitmap (2).jpg');
        ...
        $user->setAvatar('bitmap-4-1747064852.jpg');
        ...
        // Second client avec avatar
        $user2 = new User();
        $user2->setFirstName('Marie');
        $user2->setLastName('Bernard');
        $user2->setAvatar('bitmap-7-1750159823.jpg');
        $user2->setEmail('client2@cinephoria.fr');
        $user2->setPassword($this->passwordHasher->hashPassword($user2, 'userpass'));
        $user2->setAddress('5 place du 7e art, Marseille');
        $user2->setPostalCode('13001');
        $user2->setCity('Marseille');
        $user2->setCountry('France');
        $user2->setPhone('0612334455');
        $user2->setCreatedAt(new DateTimeImmutable());
        $user2->setIsActive(true);
        $user2->setRoles(['ROLE_USER']);
        $user2->setRole($this->getReference('role_user', Role::class));
        $manager->persist($user2);
        $this->addReference('user-client-2', $user2);
