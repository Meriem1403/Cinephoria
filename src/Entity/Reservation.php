...
class Reservation
{
    ...
    public function __toString(): string
    {
        ...
        $date = $this->reservationDate?->format('d/m/Y H:i') ?? '';
        $user = trim(($this->user?->getFirstName() ?? '') . ' ' . ($this->user?->getLastName() ?? ''));
        $movie = $this->showtime?->getMovie()?->getTitle() ?? '';
        $parts = array_filter(["#{$this->id}", $date, $user, $movie]);
        return implode(' — ', $parts) ?: "Reservation #{$this->id}";
    }
}
